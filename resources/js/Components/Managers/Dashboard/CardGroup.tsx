...
export default function CardGroup({ totalUsers, totalTransactions }) {
    ...
    return (
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-4">
            {data.map((card) => (
                <div
                    className="bg-white px-32 py-2 flex justify-between items-center border border-slate-300 shadow-md rounded-md hover:transform hover:scale-105 ease-in duration-300"
                    key={card.id}
                >
